...
if (__name__ == "__main__") :
  ...
  explain_user_input = '''
  Method 1
  Usage: python run_all.py <mrc list>
  Example: python /Users/kimd999/Dropbox/script/python/cryoEM/AutoMicroED/run_all.py mrc_file.list
  
  Method 2
  Usage: python run_all.py <mrc list> <args list>
  Example: python /Users/kimd999/Dropbox/script/python/cryoEM/AutoMicroED/run_all.py mrc_file.list args_file.txt
    
  Note
  Exemplar mrc_file.list would have \n\t/Users/kimd999/research/microED/microED_worskhop_test_data/ketone/raw/2_20190913_162354.mrc\n\t/Users/kimd999/research/microED/microED_worskhop_test_data/ketone/raw/3_20190913_163300.mrc
  This mrc_fie.list can be generated by for example, (base) WE39390:raw kimd999$ ls $PWD/*.mrc > mrc_file.list
  AutoMicroED will use all mrc files in mrc_file.list until it reaches > 95% completeness.
    
  If a user provided <args list> file as well, then arguments in there will be used automatically rather than waiting for user input interactively.
  
  Exemplar args.list files are in AutoMicroED/templates/args_file'''
  if (len(args) < 2):
  ...
  if (str(sys.version[:1]) == "2"):
    ...
    print ("\tIf a user wants to use alias, then put -> alias MicroED=\"/home/kimd999/bin/miniconda3/bin/python3.8 /gpustorage/automation/MicroED/Scripts/AutoMicroED/run_all.py\"")
  ...
  ############### <end> count columns, NX, NY, sections of the FIRST input mrc
    ...
    if ("ORGX" not in args_dict) or ("ORGX" not in args_dict):
  ...
  if (args_dict['input_list_has_mrc'] == True):
      ...
      for mrc_file in mrc_file_list_opened:
        ...
        if (platform.system() == "Linux"):
          ...
          ######### end of args_dict['PNNL_HPC_Cascade_sbatch'].upper() == "FALSE")
            ...
            print ("\nnumber_of_sections_in_each_mrc_file:" + str(number_of_sections_in_each_mrc_file))
          
            if (int(number_of_sections_in_each_mrc_file) > 1):
              mrc_file_basename = os.path.basename(mrc_file)
              write_this = "\nmrc_file_basename:" + str(mrc_file_basename)
              flog(write_this, args_dict['logfile_name_w_abs_path'])
              
              mrc_file_basename_splited_by_ = mrc_file_basename.split("_")
              print ("mrc_file_basename_splited_by_:" + str(mrc_file_basename_splited_by_))
              if (len(mrc_file_basename_splited_by_) == 1):
                ...
                if (len(mrc_file_basename_splited_by_) > 1):
                  ...
                  intermediate_output_folder_name = mrc_file_basename_splited_by_[0]
              print ("intermediate_output_folder_name:" + str(intermediate_output_folder_name))
    ...
    ############### <begin> single frame mrc
      ...
      #path_before_opening_list_of_mrc_file_or_smv_folder = os.getcwd()
      print ("path_before_opening_list_of_mrc_file_or_smv_folder: " + str(path_before_opening_list_of_mrc_file_or_smv_folder))
