...
def args_file_parse(args_dict):
  ...
  for line in f_in:
    ...
    elif (splited_line[0] == "mrc2smv_by_ImageMagick"):
      args_dict['mrc2smv_by_ImageMagick'] = splited_line[1]

    elif (splited_line[0] == "mrc2smv_by_PIL"):
      args_dict['mrc2smv_by_PIL'] = splited_line[1]

    elif (splited_line[0] == "mrc2smv_by_wcen"):
      args_dict['mrc2smv_by_wcen'] = splited_line[1]
...
def per_each_mrc_smv_file_both_single_and_multiple_sections (args_dict, mrc_w_path, output_folder_name):
  ...
  if (args_dict['input_list_has_mrc'] == True):
    ...
    if (('mrc2smv_by_PIL' in args_dict.keys()) and \
        str(args_dict['mrc2smv_by_PIL']).upper() == "TRUE"):
        mrc2smv.run_mrc2smv_by_PIL(args_dict, mrc_w_path, output_folder_name)
    else:
      if ('mrc2smv_by_wcen' not in args_dict.keys()) \
        or str(args_dict['mrc2smv_by_wcen']).upper() == "FALSE":
          return_of_run_mrc2smv_by_tvips = mrc2smv.run_mrc2smv_by_tvips(args_dict, mrc_w_path, output_folder_name)
          if (return_of_run_mrc2smv_by_tvips == -999):
            return -999
      else:
        mrc2smv.run_mrc2smv_by_wcen(args_dict, mrc_w_path, output_folder_name)
  ...
  if ('Bypass_image_inspection' not in args_dict.keys()) \
    ...
    if (bypass_can.rstrip() == "bypass"):
      ...
      f_out.write(write_this)
      f_out.close()
## end of def per_each_mrc_smv_file_both_single_and_multiple_sections (args_dict, mrc_w_path, output_folder_name):  


def xds_retrieve_UNIT_CELL_per_each_mrc_file_both_single_and_multiple_sections (args_dict, mrc_w_path, output_folder_name):
...
#################### end of def xds_retrieve_UNIT_CELL_per_each_mrc_file_both_single_and_multiple_sections(mrc_w_path)
